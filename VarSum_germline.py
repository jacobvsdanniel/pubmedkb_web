...
class GermlineVarSum:
    ...
    def preprocessing(self, sample):
        keys = ['VEP_VEP-refseq-Gene-Name', 'VEP_VEP-refseq-HGVSc', 'Otherinfo_FORMAT-GT', 'VEP_VEP-refseq-Consequence', 'VEP_VEP-refseq-HGVSp',
    ...
    def template_zhtw(self, table):
        ...
        if table['gene_name'] != ".":
            if table['HGVSc'] == "." : # without HGVSc annotation
                if table['genotype'] == "": # without genotype information
                    text_zhtw['gene_name'] = [f"此個案之{table['gene_name']}基因序列含有{table['consequence_zhtw']}變異，",
                                                   f"檢測結果顯示，{table['gene_name']}基因序列含有{table['consequence_zhtw']}變異，",
                                                   f"在此個案中，有一個{table['consequence_zhtw']}變異在{table['gene_name']}基因中被檢測出來，"]
                else:
                    text_zhtw['gene_name'] = [f"此個案之{table['gene_name']}基因序列含有{table['consequence_zhtw']}變異，被偵測到發生{table['genotype_zhtw']}核苷酸序列變化，",
                ...
                if table['genotype'] == "": # without genotype information
                    text_zhtw['gene_name'] = [f"此個案之{table['gene_name']}基因序列（{table['reference']}）{tmp_position}發生{table['consequence_zhtw']}變異（{table['HGVSc']}）。",
                    ...
                    text_zhtw['gene_name'] = [f"此個案之{table['gene_name']}基因序列（{table['reference']}）{tmp_position}（位於{table['exon_or_intron_rank_zhtw']}中）發生{table['consequence_zhtw']}變異，發生{table['genotype_zhtw']}核苷酸序列變化（{table['HGVSc']}）。",
        ...
        text_zhtw['ClinVar_record'] = [f"此變異於ClinVar資料庫中之致病性紀錄{table['ClinVar_record_zhtw']}。",
        ...
        text_zhtw['gnomAD_freq'] = [f"根據Genome Agrregation Database（gnomAD）之紀錄，此變異的「對偶基因頻率」（allele frequency）於東亞地區族群中為{table['gnomAD_freq']}；",
        ...
        text_zhtw['TaiwanBiobank_freq'] = [f"在臺灣人體生物資料庫（Taiwan BioBank）中，此變異的「對偶基因頻率」（allele frequency）於臺灣族群中為{table['TaiwanBiobank_freq']}。\n",
        ...
        if table['pathogenicity'] == 'recorded': # check if these is record from any of the pathogenicity prediction software       
            text_zhtw['pathogenicity'] = ["透過預測軟體對此變異之致病性進行評估，結果顯示：",
    ...
    def template_na(self):
        ...
        self.text_na_zhtw['ClinVar_record'] = ["此變異之致病性在ClinVar資料庫尚未被描述。",
        ...
        self.text_na_zhtw['gnomAD_freq'] = ["Genome Aggregation Database（gnomAD）中無此變異在東亞地區族群中的「對偶基因頻率」（allele frequency）紀錄；",
        ...
        self.text_na_zhtw['TaiwanBiobank_freq'] = ["在臺灣人體生物資料庫（Taiwan BioBank）中，此變異於臺灣族群中的「對偶基因頻率」（allele frequency）未被紀錄，顯示此變異位點在人群中相當罕見。\n",
    ...
    def generate_report(self, attribute_out = None):
        self.report = {}
        self.report['En'] = ''
        self.report['Zh'] = ''
        ...
        for key in attribute_out:
            if self.table[key] == '.':
                n = random.sample(list(range(len(self.text_na_zhtw[key]))),1)[0]
                self.report['Zh'] += self.text_na_zhtw[key][n]
                n = random.sample(list(range(len(self.text_na[key]))),1)[0]
                self.report['En'] += self.text_na[key][n]
            else:
                n = random.sample(list(range(len(self.text_zhtw[key]))),1)[0]
                self.report['Zh'] += self.text_zhtw[key][n]
                n = random.sample(list(range(len(self.text_enus[key]))),1)[0]
